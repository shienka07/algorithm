...
점심 식사에는 삼백 이십개가 적당하다 캐도, 오늘 같은 날이나 술하고 같이 낼 때는 이백 팔십개만 해라, 
어이? 배 안부르구로"
주영: "어디서 또 이상한거 배워왔냐"
...
너는 '재벌집 막내아들'의 진양철 회장님의 말에 따라 술하고 같이 초밥을 먹거나 점심 식사가 아닐 때는 초밥의 밥알을 280개로 하며,
...
입력
첫 번째 줄에 시간을 의미하는 정수 T (0 <= T <= 23)와 
술의 유무를 의미하는 정수 S (0 <= S <= 1)가 공백으로 구분되어 주어진다. 
T가 11이하이면 아침 시간, 12이상 16이하이면 점심 시간, 그렇지 않으면 저녁 시간을 뜻한다. 
